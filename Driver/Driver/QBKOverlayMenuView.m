...
- (void)addButtonWithImage:(UIImage *)image index:(NSInteger)index
{
    ...
    //CGFloat tamHuecoBoton = ( (QBK_OVERLAY_MENU_VIEW_WIDTH + QBK_OVERLAY_MENU_VIEW_HORIZONTAL_MARGINS * 2 + QBK_OVERLAY_MENU_CONTENT_VIEW_PADDING * 2 )) / QBK_OVERLAY_MENU_MAX_ADDITIONAL_BUTTONS;
   ...
   // CGFloat posXBoton = (QBK_OVERLAY_MENU_MAX_ADDITIONAL_BUTTONS - ([_additionalButtons count] + 1)) * tamHuecoBoton + posXBotonCentrado;
    CGFloat posXBoton = 7+ ([_additionalButtons count] ) * 42;

    
    // Configuramos el botón
    UIButton *newButton ;//
    //= [[UIButton alloc] initWithFrame:CGRectMake(posXBoton, posYBotonCentrado, QBK_OVERLAY_MENU_ADDITIONAL_BUTTONS_WIDTH, QBK_OVERLAY_MENU_ADDITIONAL_BUTTONS_HEIGHT)];
    
    NSLog(@"addbotton:%@",NSStringFromCGRect(CGRectMake(posXBoton, posYBotonCentrado, 44, QBK_OVERLAY_MENU_ADDITIONAL_BUTTONS_HEIGHT)));
    newButton =[UIButton buttonWithType:UIButtonTypeRoundedRect];

    newButton.frame=CGRectMake(posXBoton, 0, 44, 44);
    
    //[newButton setBackgroundImage:image forState:UIControlStateNormal];
    [newButton setTitle:@"ffjbjbbhj" forState:UIControlStateNormal];
...
- (void)addButtonWithTitle:(NSString *)title index:(NSInteger)index
{
    if (!_additionalButtons) _additionalButtons = [[NSMutableArray alloc] init];

    CGFloat posXBoton = 7+ ([_additionalButtons count] ) * 213;

    UIButton *newButton ;
    
    newButton =[UIButton buttonWithType:UIButtonTypeRoundedRect];

    newButton.frame=CGRectMake(posXBoton, 5, 203, 34);
...
- (void)setupMainButton
{
    ...
    NSLog(@"mainbotton:%@",NSStringFromCGRect(CGRectMake(x, y, QBK_OVERLAY_MENU_VIEW_MAIN_BUTTON_WIDTH, QBK_OVERLAY_MENU_VIEW_MAIN_BUTTON_HEIGHT)));
...
- (CGRect)createFoldedContentViewFrameForPosition:(QBKOverlaMenuViewPosition)position
{
    CGRect frame = CGRectMake([self bounds].origin.x+42, [self bounds].origin.y, 0, QBK_OVERLAY_MENU_VIEW_HEIGHT);
    NSLog(@"contentview:%@",NSStringFromCGRect(frame));
...
- (CGRect)createUnfoldedMainFrameForPosition:(QBKOverlaMenuViewPosition)position
{
    ...
    switch (_position) {
        ...
        default:
            //frame = CGRectMake(QBK_OVERLAY_MENU_VIEW_HORIZONTAL_MARGINS, [[self superview] bounds].size.height - (QBK_OVERLAY_MENU_VIEW_HEIGHT + QBK_OVERLAY_MENU_VIEW_BOTTOM_MARGIN + _offset.bottomOffset), [[self superview] bounds].size.width - QBK_OVERLAY_MENU_VIEW_HORIZONTAL_MARGINS * 2, QBK_OVERLAY_MENU_VIEW_HEIGHT);
            frame = CGRectMake(5,5,470,44);
            break;
    }
  //   NSLog(@"%@",NSStringFromCGRect(frame));
    return frame;
}
-(void) setMenuUnFolded:(BOOL) unfold
{
    if (!unfold) {
        [self unfoldWithAnimationDuration:0];
    } else {
        [self foldWithAnimationDuration:0];
    }
}
...
{
    if (!_unfolded) {
        [self unfoldWithAnimationDuration:0.5];
    } else {
        [self foldWithAnimationDuration:0.5];
    }
}
